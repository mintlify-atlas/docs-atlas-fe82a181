---
title: "Products API — Catalog, Stock, and Bulk Import"
sidebarTitle: "Productos"
description: "CRUD de productos en /api/productos con búsqueda por texto, alertas de stock bajo e importación masiva con upsert por código."
---
...
<ParamField body="categoriaId" type="integer">
  ID de la categoría. Si es `null` y tampoco se envía `categoriaNombre`, el sistema lanzará un error o usará la categoría por defecto en `/importar`.
</ParamField>
...
Mismos campos que `POST /api/productos`. El campo `stockActual` es ignorado por el servidor en este endpoint.
